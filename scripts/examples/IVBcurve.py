...
attributes['magnet wait time / s'] = 5

coordinates=[
            {'name':'Zmagnet / T','range':np.linspace(0,0.1,100)},
            {'name':'Repeats Curve','range':[1,2,3]},
            {'name':'Dac 1 / mV','range':np.linspace(-1000,1000,300)},
             ]
def setupFunction():
    devices['ivvi'].set_dacs_zero()
    devices['Zmagnet'].set_field(0.0)
    time.sleep(attributes['magnet wait time / s']) # seconds
    return {}
...
def acqFunction(coordinates,results,prev_coordinates,prev_result,user_storage):
    ...
    if prev_coordinates['Zmagnet / T'] != coordinates['Zmagnet / T']:
        ...
        time.sleep( attributes['magnet wait time / s'] )
